...
Monitors the primary LLM endpoint (professionalize.LLM) health across calls
(latency, error rate, consecutive failures) and proactively routes to the
configured fallback (Ollama) when the primary is detected as flaky.

Ported from foss-launcher's implementation with no external dependencies.
...
State machine:
    CLOSED   → Normal: calls go to primary endpoint (professionalize.LLM)
    OPEN     → Flaky detected: calls go to fallback directly (Ollama)
    HALF_OPEN → Recovery probe: one call to primary to test recovery

Transitions:
    CLOSED  → OPEN:      3+ consecutive failures, OR >50% error rate in window,
                         OR avg latency > 30s
    OPEN    → HALF_OPEN: after recovery_timeout_s (default 60s) in OPEN state
...
class CircuitBreaker:
    ...
    Usage pattern::
        ...
        if cb.should_use_fallback():
            # route to fallback directly (Ollama)
        else:
            try:
                result = call_primary()
    ...
    def should_use_fallback(self) -> bool:
        ...
        with self._lock:
            if self._state == CircuitState.OPEN:
                # Check recovery timeout — may advance to HALF_OPEN
                if (
    ...
    def record_failure(self, latency_s: float) -> None:
        ...
        Permanent failures (4xx auth errors) MUST NOT be passed here —
        use ``_is_transient_error()`` to classify before calling this.
    ...
    def get_status(self) -> Dict:
        """Return a snapshot of current circuit breaker state for logging/telemetry."""
        with self._lock:
...
def build_circuit_breaker_from_config(
    ...
    """Construct a CircuitBreaker from the ``model_routing.circuit_breaker`` config section.

    Args:
        cb_cfg: The ``circuit_breaker`` sub-dict from ``model_routing`` config
                (may be empty ``{}`` — safe to pass model_dump output).
        has_fallback: True if a fallback endpoint (Ollama) is configured.

    Returns:
        ``CircuitBreaker`` instance if enabled, ``None`` otherwise.
        Returns None if has_fallback is False (no circuit breaker needed).
    """
