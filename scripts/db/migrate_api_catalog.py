...
# Schema V3 additions (2026-02-18) - Cross-family sharing + version tracking
SCHEMA_V3_COLUMNS = [
    # (table, column, type, default)
    ("learned_patterns", "scope", "TEXT", "'family'"),  # 'family' | 'global'
    ("learned_patterns", "schema_version", "INTEGER", "2"),  # Schema version for future migrations
]


def upgrade_schema_v3(db_path: Path) -> int:
    """Add V3 columns for cross-family sharing and version tracking (idempotent)."""
    conn = sqlite3.connect(str(db_path))
    added = 0

    for table, column, col_type, default in SCHEMA_V3_COLUMNS:
        try:
...
def create_schema(db_path: Path) -> None:
    ...
    # Apply V3 schema upgrades (cross-family sharing + version tracking)
    added = upgrade_schema_v3(db_path)
    if added > 0:
        logger.info(f"Schema V3: Added {added} new columns for cross-family sharing")
